	...
	do
						...
						local input = frame.InputBegan:connect(function(key)
							if key.UserInputType == Enum.UserInputType.MouseButton1 then
								...
								do
									pcall(function()
										frame:TweenPosition(
											UDim2.new(0, mouse.X - objectPosition.X, 0, mouse.Y - objectPosition.Y),
											"Out",
											"Linear",
											0.1,
											true
		...
		do
			...
			function types.window(name, options)
				...
				local newWindow = library:Create("Frame", {
					...
					library:Create("Frame", {
						...
						BackgroundColor3 = (
							options.underlinecolor ~= "rainbow" and options.underlinecolor or Color3.new()
						),
						BorderSizePixel = 0,
						ZIndex = 3,
					}),
				...
				newWindow:FindFirstChild("window_toggle").MouseButton1Click:connect(function()
					...
					window.container:TweenSize(targetSize, targetDirection, "Quint", 0.3, true)
					wait(0.3)
			...
			function types:Toggle(name, options, callback)
				...
				assert(options.flag, "Toggle requires a flag")
				...
				local check = library:Create("Frame", {
					...
					library:Create("TextLabel", {
						...
						library:Create("TextButton", {
							Name = "Checkmark",
							Text = default and utf8.char(10003) or "",
							Font = library.options.font,
							TextSize = library.options.fontsize,
				...
				local label = check:FindFirstChild(name)
				local button = label:FindFirstChild("Checkmark")

				local function set(value)
					location[options.flag] = value
					button.Text = value and utf8.char(10003) or ""
					callback(value)
				end

				button.MouseButton1Click:Connect(function()
					set(not location[options.flag])
				end)

				library.callbacks[options.flag] = function()
					set(not location[options.flag])
				end

				-- Fire callback ONCE for default
				if default then
					callback(true)
				end
			...
			function types:Label(name)
				...
				local func = {}
				function func:Set(new)
					label.Text = new
				end

				func.Refresh = func.Set
				return func
			end
			...
			function types:SearchBox(text, options, callback)
				...
				local function rebuild(text)
							...
							if string.sub(string.lower(v), 1, string.len(text)) == string.lower(text) then
								...
								button.MouseButton1Click:connect(function()
									...
									box:FindFirstChild("Box").Container
										:TweenSize(UDim2.new(1, 0, 0, 0), "Out", "Quint", 0.3, true)
								end)
							end
						end
					end
					...
					box:FindFirstChild("Box").Container:TweenSize(UDim2.new(1, 0, 0, y), "Out", "Quint", 0.3, true)
			...
			function types:Dropdown(name, options, callback)
				...
				button.MouseButton1Click:connect(function()
					...
					container:TweenSize(goSize, "Out", "Quint", 0.3, true)
					...
					input = game:GetService("UserInputService").InputBegan:connect(function(a)
						if a.UserInputType == Enum.UserInputType.MouseButton1 and (not isInGui(container)) then
							...
							container:TweenSize(UDim2.new(1, 0, 0, 0), "In", "Quint", 0.3, true)
							wait(0.15)
				...
				local function reset(self, list)
					options = array
					location[flag] = array[1]
					pcall(function()
						input:disconnect()
